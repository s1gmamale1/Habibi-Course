...
2. Audio tier assignment per gap-1 §5: `youtube-cue` where the syllabus names a per-letter Arabic101 makharij video or a Muallimi Soniy timestamp; otherwise `teacher-voice` with the `cue` line taken from that letter's listen-for/articulation text. No `qari-clip` in Phase 1 (no Quran words yet).
