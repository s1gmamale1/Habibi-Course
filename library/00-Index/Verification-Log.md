...
| 4 | يَبْصُۜطُ — sīn or ṣād | sources genuinely differ; ṣād predominates for Hafs/Shāṭibiyyah, some regional traditions read sīn. **Re-searched 2026-08-11** in [[Nihayat-al-Qawl-al-Mufid]]: `بَسْطَة`/`بَصْطَة` surfaces once in the OCR layer as an example of variation *in form, not meaning* — both readings transmitted, which is already this note's position. **Not read off the scan, and it would not say which Ḥafṣ reads even if confirmed** | [[Hafs-Special-Words]], held at `needs-review` until checked against a Madinah mushaf **and** a licensed teacher |
| 6 | The two muṣḥaf zeros — الصفر المستدير / المستطيل | **The tajwīd shelf was the wrong shelf**, established 2026-08-11: [[Nihayat-al-Qawl-al-Mufid]] does not treat them at all (`الصفر` and `المستدير` occur zero times; the two `المستطيل` hits are the *istiṭālah* of ḍād). Marking the muṣḥaf is **ḍabṭ**, a different discipline. The classical line was located in **al-Kharrāz** (d. 718 AH, public domain) — *"فدارة لزم ذا المزيدا"*, a circle for a written-but-added letter — **but only in a degraded OCR layer of a modern in-copyright commentary**. The one full scan reachable (`archive.org/details/dalil_al7yran`) was OCR'd with a **Latin-alphabet model** and holds zero Arabic characters, so it cannot be searched | [[Silent-Letters]]. Needs a legible **متن الذيل في ضبط القرآن**, or a citation to the muṣḥaf's own printed legend |
