...
export function DrillGrid({ drill }: { drill: Drill }) {
  return (
    <section className="drill glass mb-8 break-inside-avoid rounded-2xl p-4 sm:p-5">
      {drill.title && <h3 className="mb-1 text-xl font-semibold text-white/90">{drill.title}</h3>}
      {drill.instructions && <p className="mb-4 text-white/60">{drill.instructions}</p>}
